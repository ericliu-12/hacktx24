...
const ThreeScene: React.FC<ThreeSceneProps> = ({ startAnim, setStartAnim }) => {
    ...
    const animateCamera = () => {
        ...
        function animate(time: number) {
            ...
            cameraRef.current!.position.lerpVectors(initialPosition, targetPosition, easeInOutSine(progress));
            cameraRef.current!.rotation.x = lerp(initialRotationX, targetRotationX, easeInOutSine(progress));

            if (cabinetRef.current) {
                const targetCabinetRotationY = Math.round(startingCabinetRotationY / (2 * Math.PI)) * (2 * Math.PI) - (Math.PI / 2);
                cabinetRef.current.rotation.y = lerp(startingCabinetRotationY, targetCabinetRotationY, easeInOutSine(progress));
            }
